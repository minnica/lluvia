...
Estado: **A0 y A1 terminadas; A2 y A3 publicadas y probadas con una ruta real de TomTom, pendientes de pruebas funcionales en teléfono; A4 publicada, pendiente de aceptación operativa; código A5 publicado, activación de Weatherbit pospuesta por decisión del usuario** (23 de septiembre de 2026). Contexto general en [README](../README.md), acuerdo técnico en [contratos de proveedores](provider-contracts.md) y pasos pendientes en [operación](operations.md).
...
| A2 🟡 | Rutas y favoritos | A1 y credenciales/licencias geográficas | Mapa y ruta real de 18,2 km/34 min observados en producción; faltan búsqueda, favoritos y prueba en teléfono. |
| A3 🟡 | Comparación de salidas | A2 | Cuatro salidas reales observadas en producción; falta validar interacción móvil y un caso de cambio de ruta. |
| A4 🟡 | MVP diario estabilizado | A1–A3 y comprobación funcional | Código publicado; faltan prueba móvil, estados de error, cuotas y revisión de logs. |
| A5 ⏸️ | Evoluciones y sustitución de proveedor | Necesidad concreta y adaptador/configuración disponibles | Código Weatherbit publicado; activación pospuesta. No bloquea el cierre del Track A inicial con Open-Meteo. |
...
**Estado al 22 de septiembre de 2026: A1 aceptada.** Están implementados el adaptador Open-Meteo horario, `/api/weather`, evaluación prudente, pantalla móvil, ubicaciones/preferencias en IndexedDB v1, manifest, iconos y service worker. Pasan lint, tipos, 10 pruebas y build con Webpack; Turbopack quedó bloqueado por un puerto interno del sandbox. La primera versión verificada de [Vercel](https://lluvia-steel.vercel.app/) en `minnicas-projects/lluvia` usó el commit `028a972` de `main`. Con la conexión reautorizada se comprobaron portada, manifest, service worker e iconos (`200`), el rechazo de parámetros inválidos (`400`) y respuestas reales de `/api/weather` para 30, 60 y 180 minutos (`200`, `open-meteo`, punto `ok`, `no-store`). La consulta de errores de runtime no encontró errores. El usuario aportó capturas de una consulta vigente en su teléfono y de la reapertura de la app instalada sin Wi-Fi ni datos móviles: el pronóstico anterior seguía visible, fechado y sin recomendación actual. La cuadrícula de la respuesta para las coordenadas iniciales quedó a unos 8,2 km; la indicación de distancia se publicó después con A3. La respuesta real confirma acceso y cobertura temporal, no precisión local. Los [seguimientos](../README.md#validación-de-a1-y-seguimientos) no bloquean la aceptación de A1.
...
**Verificado:** lint, tipos, 19 pruebas de A1–A3 y build Webpack pasan. Las pruebas sintéticas cubren lluvia al inicio y final, ausencia de señal, valores nulos, horizonte corto, una alternativa sin ruta, cambios de geometría/tiempo y reutilización de 10 puntos meteorológicos en cuatro salidas idénticas. Posteriormente, el usuario mostró una comparación real con TomTom y Open-Meteo en producción: ruta de 18,2 km/34 min y cuatro salidas ahora, +10, +20 y +30 min. No se ha comprobado visualmente en teléfono ni un cambio de geometría; la aceptación funcional completa de A2 y A3 sigue pendiente. A4 añadió límites y caché; véase [operación](operations.md).
...
**Implementación local del 22 de septiembre de 2026:** las cuatro salidas admiten como máximo 64 puntos meteorológicos distintos, además de los 24 tramos máximos por ruta y cuatro horas de duración. Los endpoints aplican un guardia de ráfagas por instancia, con `429` y `Retry-After`; la protección global y el gasto deben configurarse en la plataforma y en proveedores. La consulta Open-Meteo horaria tiene caché de servidor de 300 segundos mediante `fetch` de Next.js; la respuesta pública conserva `no-store`, fecha de recuperación y regla de vigencia de 20 minutos. Se registran resultado, latencia y cota de consumo sin ubicación ni secretos. La búsqueda TomTom distingue timeout y cuota. Se ampliaron tamaños táctiles y se subió el service worker a v4 sin cambiar IndexedDB v2. [Configuración, límites y lista de aceptación](operations.md).

**Pendiente para cerrar A4:** el usuario configuró claves TomTom y observó en producción el mapa, Routing y las cuatro salidas para San Rafael–Temamatla (18,2 km, 34 min). Falta probar geocodificación, permisos denegados, favoritos, comparación en teléfono y actualización de la PWA. La precisión local continúa provisional. El código A5 está publicado según la captura de Vercel, pero Weatherbit sigue sin activar.
...
**Decisión del 23 de septiembre de 2026:** el usuario pospone Weatherbit y continúa con Open-Meteo. Si observa imprecisiones, registrar episodios con hora, lugar, pronóstico y lluvia observada; comparar causas y alternativas antes de asumir que Weatherbit mejorará el resultado. La validación real de Weatherbit deja de ser un pendiente para cerrar el Track A inicial.

**Implementación publicada el 22 de septiembre de 2026, sin activar Weatherbit:** `WeatherbitProvider` usa `/v2.0/forecast/hourly` con unidades métricas y devuelve el contrato `WeatherResponse` que consumen la consulta local y la ruta. `WEATHER_PROVIDER=weatherbit` activa ambos endpoints; `open-meteo` permanece como valor predeterminado y rollback. La UI toma fuente y definición del evento de probabilidad de la respuesta, sin afirmar para Weatherbit el umbral de 0.1 mm propio de Open-Meteo. La caché de `fetch` está separada por URL/producto y dura 300 s. Un pronóstico anterior en IndexedDB sigue fechado y sin recomendación actual tras cambiar de proveedor. El service worker sube a v5; favoritos e IndexedDB siguen en v2.

Los fixtures sintéticos cubren horas precedentes, unidades, nulos, cuotas, clave ausente y selección por configuración. Pasaron lint, tipos, 26 pruebas y build Webpack; el primer build tras los controles falló de forma transitoria al leer `tsc --showConfig`, y el build aislado pasó. **Pendiente para aceptar el cambio operativo:** [Weatherbit Hourly no figura en el plan Free permanente](https://www.weatherbit.io/pricing) y esta app guarda pronósticos, para lo que [Weatherbit exige suscripción de pago activa](https://help.weatherbit.io/faq/can-i-store-data-retrieved-from-the-api-locally/); confirmar acceso y licencia, probar una respuesta real en San Rafael y en una ruta, revisar la semántica de la probabilidad horaria y la discrepancia entre dos textos de Weatherbit sobre la ventana de `precip`, medir cobertura/cuotas, activar primero en preview y luego revertir o promover por configuración. A2–A4 siguen con aceptación funcional parcial. [Pasos operativos](operations.md#cambio-de-proveedor-a5).
