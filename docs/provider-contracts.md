...
Estado: tipos y validadores Zod de A0 implementados en `src/domain/provider-common.ts`, `src/domain/weather/contracts.ts` y `src/domain/routing/contracts.ts`; factorías de servidor en `src/server/providers`. A1 añadió el adaptador HTTP Open-Meteo; la API de producción devolvió respuestas reales normalizadas para San Rafael con periodos de 30, 60 y 180 minutos y estado `ok`, lo que confirma cobertura técnica, no precisión meteorológica local. A2 añadió el adaptador TomTom y evaluación por tramo; A3 añadió la comparación de cuatro salidas y reutilización de series meteorológicas. El usuario observó después en producción una ruta real TomTom de 18,2 km/34 min y cuatro salidas con Open-Meteo, aunque siguen pendientes pruebas móviles y de geocodificación. A5 añadió Weatherbit Hourly como adaptador alternativo publicado, con solo pruebas sintéticas; el usuario pospuso activarlo el 23 de septiembre de 2026. Las interfaces siguen compartidas con el futuro recolector independiente. No se añadió framework de plugins ni microservicios.
...
El plan Free permanente no incluye Hourly Forecast y la [FAQ de Weatherbit exige suscripción de pago activa para almacenamiento local](https://help.weatherbit.io/faq/can-i-store-data-retrieved-from-the-api-locally/). Este adaptador usa caché de servidor y la app conserva una copia en IndexedDB; una prueba gratuita de acceso no autoriza por sí sola activar el flujo completo.
