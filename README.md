...
**A2 publicada; aceptación con TomTom pendiente (22 de septiembre de 2026):** `/routes` integra routing, geocodificación, mapa, exposición horaria por tramo y favoritos con exportación/importación. En la última revisión registrada no había claves TomTom en Vercel ni en este entorno y el sandbox impedía abrir un servidor local, por lo que no se ha comprobado un recorrido real ni la interfaz en teléfono. El último despliegue verificado incluye A2/A3 en el commit `9ae1bd0`; la indicación de distancia de A1 también está en esa copia. Detalle y pasos de aceptación en [A2](docs/app-plan.md#a2--recorridos-y-favoritos).

**A3 publicada; validación real pendiente (22 de septiembre de 2026):** `/routes` compara ahora, +10, +20 y +30 minutos con rutas propias y un lote meteorológico compartido. Muestra cobertura, margen de paso ±5/10 minutos y cambios de ruta o tiempo. Con Open-Meteo horario no recomienda una espera como mejor por diferencias que la fuente no puede resolver. El último despliegue verificado, `dpl_9AiLJZTMJZvEVk73nKjm1sxd77WJ`, figuraba `READY` en Vercel y corresponde a `9ae1bd0` de `main`; sigue pendiente comprobar A2/A3 con TomTom real y teléfono. Detalle en [A3](docs/app-plan.md#a3--comparación-de-salida).

**A4 endurecida localmente; aceptación pendiente (22 de septiembre de 2026):** hay caché de servidor de cinco minutos para Open-Meteo horario, tope de 64 puntos meteorológicos por comparación, guardias de ráfagas por instancia, respuestas `429` con tiempo de espera y registros de latencia/resultados sin ubicaciones. La PWA sube su caché de interfaz a v4; IndexedDB sigue en v2. Pasan lint, tipos, 20 pruebas y build. En Vercel solo aparecen `WEATHER_PROVIDER` y `ROUTING_PROVIDER`: faltan las claves TomTom para validar rutas reales y revisar la interfaz en teléfono. La herramienta de publicación rechazó el despliegue porque requiere aprobación no disponible en esta sesión. [Guía de operación y aceptación](docs/operations.md).

**A5 implementada localmente; activación real pendiente (22 de septiembre de 2026):** Weatherbit Hourly ya cumple el contrato meteorológico de la app y puede seleccionarse para consulta local y rutas con `WEATHER_PROVIDER=weatherbit` y `WEATHERBIT_API_KEY` en servidor. Open-Meteo sigue siendo el predeterminado y la opción de reversión. La interfaz muestra la fuente y el evento de probabilidad de la respuesta; la caché de interfaz sube a v5, sin migrar favoritos. Pasan lint, tipos, 26 pruebas sintéticas y build Webpack. No hay credenciales Weatherbit ni verificación de una respuesta real o de licencia; A5 no está publicada. [Cambio de proveedor y validación pendiente](docs/operations.md#cambio-de-proveedor-a5).
...
La UI funciona con datos horarios y explica cuándo no puede estimar inicio exacto o una ventaja por esperar. Weatherbit Hourly está implementado como alternativa local pendiente de validación real; OpenWeather continúa como candidato sin adaptador de aplicación.
...
- Abre `/routes` o «Abrir recorridos» desde la portada. Introduce origen, destino y hasta cuatro paradas por coordenadas; con claves configuradas, también puedes buscarlos en México o elegirlos en el mapa. Selecciona motocicleta (beta de TomTom) o automóvil y preferencias de vías. «Comparar cuatro salidas» calcula rutas con tiempos acumulados y consulta Open-Meteo en los puntos de paso. Muestra cada tramo y su hora, señal horaria, cobertura y distancia a la cuadrícula. Las franjas coloreadas y la lista son seleccionables entre sí. Sin `TOMTOM_API_KEY`, la consulta local de A1 sigue funcionando.
...
- Siguiente paso: configurar claves TomTom y límites de gasto, confirmar licencia, validar [A2](docs/app-plan.md#a2--recorridos-y-favoritos), [A3](docs/app-plan.md#a3--comparación-de-salida) y [A4](docs/app-plan.md#a4--mvp-diario-y-operación) con ruta real y teléfono; validar A5 con acceso Weatherbit en preview si se decide promoverlo; publicar la versión validada y verificar por HTTPS. [Lista operativa](docs/operations.md).
