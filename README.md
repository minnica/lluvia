...
## Recorridos de A2 en la copia local

- Abre `/routes` o «Abrir recorridos» desde la portada. Introduce origen, destino y hasta cuatro paradas por coordenadas; con claves configuradas, también puedes buscarlos en México o elegirlos en el mapa. Selecciona motocicleta (beta de TomTom) o automóvil y preferencias de vías. «Analizar recorrido ahora» calcula la ruta con tiempos acumulados y consulta Open-Meteo en los puntos de paso. Muestra cada tramo y su hora, señal horaria, cobertura y distancia a la cuadrícula. Las franjas coloreadas y la lista son seleccionables entre sí. Sin `TOMTOM_API_KEY`, la consulta local de A1 sigue funcionando.
