...
**A2 publicada; aceptación con TomTom pendiente (22 de septiembre de 2026):** `/routes` integra routing, geocodificación, mapa, exposición horaria por tramo y favoritos con exportación/importación. No hay claves TomTom en Vercel ni en este entorno y el sandbox impide abrir un servidor local, por lo que no se ha comprobado un recorrido real ni la interfaz en teléfono. El despliegue de producción actual incluye A2/A3 en el commit `9ae1bd0`; la indicación de distancia de A1 también está en esa copia. Detalle y pasos de aceptación en [A2](docs/app-plan.md#a2--recorridos-y-favoritos).

**A3 publicada; validación real pendiente (22 de septiembre de 2026):** `/routes` compara ahora, +10, +20 y +30 minutos con rutas propias y un lote meteorológico compartido. Muestra cobertura, margen de paso ±5/10 minutos y cambios de ruta o tiempo. Con Open-Meteo horario no recomienda una espera como mejor por diferencias que la fuente no puede resolver. El despliegue de producción `dpl_9AiLJZTMJZvEVk73nKjm1sxd77WJ` figura `READY` en Vercel y corresponde a `9ae1bd0` de `main`; sigue pendiente comprobar A2/A3 con TomTom real y teléfono. Detalle en [A3](docs/app-plan.md#a3--comparación-de-salida).
...
- En la copia A4, `npm run lint`, `npm run typecheck`, `npm test` y `npm run build` pasan con dependencias locales. Ejecutar tipos y build en secuencia: el build regenera `.next/types`. Build usa `next build --webpack`: Turbopack falló antes en este sandbox al intentar abrir un puerto interno para procesar CSS; la opción Webpack de Next 16 compila correctamente. `npm ci --offline` no terminó antes porque falta `picomatch` en caché; repetir con red. `next start` estuvo bloqueado por `listen EPERM`, así que la automatización de navegador para A2–A4 está pendiente; la prueba manual en Android corresponde a A1.
...
- Siguiente paso: configurar claves TomTom y límites de gasto, confirmar licencia, validar [A2](docs/app-plan.md#a2--recorridos-y-favoritos), [A3](docs/app-plan.md#a3--comparación-de-salida) y [A4](docs/app-plan.md#a4--mvp-diario-y-operación) con ruta real y teléfono; publicar A4 y verificar por HTTPS. [Lista operativa](docs/operations.md).
